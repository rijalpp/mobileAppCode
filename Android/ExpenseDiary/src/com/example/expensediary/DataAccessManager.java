...
public class DataAccessManager {
	...
	public User CreateUser(User user) {
		...
		Cursor cursor = ourDatabase.query(Table_User,
				columnsUser, User_ID + " = " + i, null,
				null, null, null);
		...
		Log.v("New UserID:  ", user.getUserID().toString());
	...
	public Category AddCategory(Category category) {
		...
		Cursor cursor = ourDatabase.query(Table_Category, columnsCategory, Category_CategoryID+"="+i, null, null, null, null);
		cursor.moveToFirst();
		 category = CursorToCategory(cursor);
		 cursor.close();
		 Log.v("Category created: "," The Category ID is: "+category.getCategoryID());
		if (i > 0)
			return category;
		else
			return null;
	...
	public List<Goal> SelectGoalsByUser( String userID) {

		List<Goal> goalList = new ArrayList<Goal>();

		
		  Cursor cursor = ourDatabase.query(Table_Goal, columnsGoal,
		  Category_UserID + "=?", new String[]
	...
	private Expense CursorToExpense(Cursor cursor) {
		...
		expense.setExpenseDate(cursor.getString(expenseExpenseDateIndex));
	...
	// RETRIEVES THE EXPENSE BY CATEGORY.
		public List<Expense> SearchExpensesByUser(String userID) {

			List<Expense> expenseList = new ArrayList<Expense>();

			Cursor cursor = ourDatabase.query(Table_Expense, columnsExpense,
					Expense_UserID + "=?", new String[] { userID }, null,
					null, null, null);
	...
	public Expense AddExpense(Expense expense) {
		...
		cv.put(Expense_ExpenseDate, expense.getExpenseDate().toString());
		...
		cv.put(Expense_UserID, expense.getUserID());
		long i = ourDatabase.insert(Table_Expense, null, cv);
		
		Cursor cursor = ourDatabase.query(Table_Expense, columnsExpense, Expense_ExpenseID+"="+i, null, null, null, null);
		cursor.moveToFirst();
		 expense = CursorToExpense(cursor);
		 cursor.close();
		 Log.v("Expense created: "," The Expense ID is: "+expense.getExpenseID());
